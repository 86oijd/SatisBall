// Frame-perfect export test. Usage: node tools/export.cjs <mode> <out> [targetLen]
...
(async () => {
  ...
  const r = await p.evaluate(async ([mode, tl]) => {
    const g = SB.test.makeGame(mode, { targetLen: +tl });
    const cfg = Object.assign({}, g.cfg); delete cfg.canvas;
    const res = await SB.recorder.exportRun(cfg, { maxSecs: 70, force: window.__force });
    const buf = new Uint8Array(await res.blob.arrayBuffer());
    let bin = ''; for (let i = 0; i < buf.length; i += 0x8000) bin += String.fromCharCode.apply(null, buf.subarray(i, i + 0x8000));
    return { b64: btoa(bin), ext: res.ext, duration: res.duration, codec: res.codec, audio: res.audio };
  }, [mode, tl]);
  fs.writeFileSync(out.replace(/\.\w+$/, '.' + r.ext), Buffer.from(r.b64, 'base64'));
  console.log(JSON.stringify({ ext: r.ext, duration: r.duration, codec: r.codec, audio: r.audio, bytes: r.b64.length * 0.75, wallSecs: (Date.now() - t0) / 1000 }));
